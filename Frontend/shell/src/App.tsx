...
function useDynamicImport({
  ...
  const loadComponent = async (isRetry: boolean = false): Promise<void> => {
    ...
    try {
      ...
      const Component = await loadRemote<React.ComponentType>(
        `${scope}/${module}`,
      );
